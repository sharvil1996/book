...
public class UserUpdateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		CustomerBean temp = (CustomerBean) session.getAttribute("customerBean");
		...
		if (isError) {
			...
			customerBean.setCustomerIsAdmin(temp.getCustomerIsAdmin());
			customerBean.setCustomerPwd(temp.getCustomerPwd());
			...
			if (new CustomerDAO().update(customerBean)) {
				...
				customerBean.setCustomerPinnumber(new PincodeDAO().getPincodeName(customerBean.getCustomerPincode()));

				session.setAttribute("customerBean",customerBean);
				request.setAttribute("msgProfile","<script>showToast('Profile Updated Successfully...!','default',5000);</script>");				
